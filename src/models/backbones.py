...
class NanoSAMBackbone(BaseBackbone):
    ...
    def __init__(self, variant: str, img_size: int, checkpoint: str | None = None, trainable: bool = False):
        super().__init__()
        if variant not in _NANOSAM_VARIANTS:
            raise ValueError(f"Unsupported NanoSAM variant={variant!r}. Expected one of {sorted(_NANOSAM_VARIANTS)}")

        info = _NANOSAM_VARIANTS[variant]
        self.img_size = int(img_size)
